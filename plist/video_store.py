...
class VideoStore:
    ...
    def __init__(self, cache_file_name='video_store_2.pkl'):
        ...
        with TT('populate Videostore'):
            for idx, path in tqdm(enumerate(candidate_paths), total=len(candidate_paths)):
    ...
    def get_file_list(self):
        with TT('sort by modification time'):
            ...
            flist.sort(key=lambda el: el.stat.st_ctime, reverse=True)  # sort by modification time, newest fist
        return flist
    ...
    def grab_files_from_disk() -> List[str]:
        ...
        """
        valid_extensions = ['.avi', '.asf', '.mp4', '.m4v', '.mov', '.flv', '.wmv', '.mkv']
        ...
        all_files = [el for el in all_files if os.path.splitext(el)[1] in valid_extensions]
