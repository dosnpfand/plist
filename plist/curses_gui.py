...
class MyCursesGUI(object):
    ...
    def update(self):
        ...
        while self.state.gui_requests:
            ...
            if request == 'candlist_left' and len(self.state.candidate_files) > self.candidates_wnd.num_lines:
                self.candidates_wnd.rotate_candlist(-1, self.state)
            elif request == 'candlist_right' and len(self.state.candidate_files) > self.candidates_wnd.num_lines:
                self.candidates_wnd.rotate_candlist(1, self.state)
...
class InputWnd(object):
    ...
    def __init__(self, loc_y=0, num_lines=1):
        self.wnd = curses.newwin(num_lines, curses.COLS, loc_y, 0)  # nlines, ncols, begin_y, begin_x
...
class Candlist(object):
    ...
    def rotate_candlist(self, direction, state):
        ...
        if direction == 1 and state.cand_offset + self.num_lines - 1 > len(state.candidate_files):
            return
    ...
    def update(self, state):
        ...
        for idx in range(min(self.num_lines - 1, len(state.candidate_files))):
            ...
            if 0 <= real_idx < len(state.candidate_files):
                self.wnd.addnstr(idx, 0, "%i: (%i / %i) %s"
                                 % (idx+1 if idx != 9 else 0, real_idx+1, len(state.candidate_files), state.candidate_files[idx]), curses.COLS - 1)

        self.wnd.addnstr(self.num_lines - 1, 0, f"{len(state.candidate_files)} files  |  duration {state.get_total_play_duration()}", curses.COLS - 1)
        self.wnd.refresh()
    ...
    def update(self, state):
        ...
        for idx in range(min(self.num_lines, len(state.play_list))):
            ...
            if idx == state.selected_line:
                self.wnd.addnstr(idx, 0, "%i: %s" % (idx, state.play_list[idx]), curses.COLS-1, curses.A_BOLD)
            else:
                self.wnd.addnstr(idx, 0, "%i: %s" % (idx, state.play_list[idx]), curses.COLS-1)
        self.wnd.refresh()
